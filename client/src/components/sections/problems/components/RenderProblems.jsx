  ...
  return (
    <div ref={containerRef}>
      ...
      <motion.div
        ...
        className={`grid gap-6 px-4 pb-12 mx-auto max-w-screen-2xl ${gridColumns}`}
      >
                ...
                {problem.tags?.length > 0 && (
                  <div className="flex items-center space-x-2">
                    {problem.tags.slice(0, 2).map((tag, index) => (
                      <span
                        key={index}
                        className="px-3 py-1 text-xs tracking-wide text-white bg-gradient-to-r from-blue-500 to-indigo-500 rounded-full shadow-lg lg:text-sm"
                      >
                        {tag}
                      </span>
                    ))}
