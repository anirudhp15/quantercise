    ...
    <div className="p-2 pt-0 bg-gray-900 rounded-b-xl">
      ...
      <div className="mt-4 mb-4 xl:mt-8 xl:mb-0">
        <div className="flex relative items-center">
          ...
          <div
            className={`flex gap-2 absolute bottom-2 left-2 z-10 items-center ${
              loadingFeedback || !userAnswer.trim() ? "block" : "hidden"
            }`}
          >
          ...
          <textarea
            ...
            className={`w-full px-4 pt-4 pb-8 text-gray-200 bg-gray-800 border rounded-lg resize-none focus:outline-none focus:ring-2 focus:ring-${
              inputType === "question"
                ? "blue"
          ...
          <button
            ...
            className={`absolute bottom-2 right-2 p-3 text-white rounded-full bg-gray-600 transition-all ${
...
const UserMessage = ({ message, currentUser, index }) => (
  ...
  >
    ...
    <div className="w-full xl:flex-1 xl:min-w-0">
      ...
      <div className="p-4 bg-gray-800 rounded-lg border border-gray-700 shadow transition-colors hover:border-gray-600">
...
const AssistantMessage = ({ message, feedbackCategory, isPro }) => {
  ...
  const MemoizedMarkdown = React.memo(({ content }) => (
    <ReactMarkdown
      ...
      components={{
        root: ({ node, ...props }) => (
          <div className="text-gray-200 markdown-content" {...props} />
        ),
        p: ({ node, ...props }) => (
        ...
        code: ({ node, inline, ...props }) => (
          <code
            className={`${
              inline ? "bg-gray-700 px-1 py-0.5 rounded text-sm" : ""
            }`}
            {...props}
          />
        ),
        pre: ({ node, ...props }) => (
          <pre
            className="overflow-x-auto p-3 my-4 bg-gray-900 rounded-md"
            {...props}
          />
        ),
        h1: ({ node, ...props }) => (
        ...
        h3: ({ node, ...props }) => (
          <h3 className="mt-4 mb-3 font-bold text-md" {...props} />
        ),
        ul: ({ node, ...props }) => (
          <ul className="pl-6 mb-4 space-y-2 list-disc" {...props} />
        ),
        ol: ({ node, ...props }) => (
          <ol className="pl-6 mb-4 space-y-2 list-decimal" {...props} />
        ),
        li: ({ node, ...props }) => <li className="mb-1" {...props} />,
        blockquote: ({ node, ...props }) => (
          <blockquote
            className="py-2 pl-4 mb-4 italic rounded-r border-l-4 border-gray-500 bg-gray-800/50"
            {...props}
          />
        ),
        table: ({ node, ...props }) => (
        ...
        th: ({ node, ...props }) => (
          <th
            className="px-3 py-2 text-sm font-medium tracking-wider text-left text-gray-300 uppercase bg-gray-800"
            {...props}
          />
        ),
        td: ({ node, ...props }) => (
          <td
            className="px-3 py-2 text-sm text-gray-400 whitespace-nowrap border-t border-gray-700"
            {...props}
          />
        ),
        a: ({ node, ...props }) => (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex flex-wrap items-center mb-1">
          <p
            className={`font-semibold ${
              message.type === "feedback"
                ? feedbackCategory.color
                : "text-blue-400"
            }`}
          >
            Qube
          </p>
          {message.type === "feedback" && feedbackCategory && (
            <div className="flex items-center ml-2">
              <span
          ...
          {message.type === "question" && (
            <div className="flex items-center ml-2">
              <span className="px-2 py-0.5 text-xs bg-indigo-900/50 text-indigo-300 rounded-full">
                ANSWER
              </span>
            </div>
          )}
        ...
        <div
          className={`p-4 rounded-lg bg-gray-800 shadow-md ${
            message.type === "feedback" && feedbackCategory
              ? `border-l-4 ${feedbackCategory.color.replace("text", "border")}`
              : message.type === "explanation"
